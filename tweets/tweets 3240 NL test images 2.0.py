...
for screen_name in screen_names[:1]:
    ...
    outtweets = [[tweet.id_str, tweet.favorite_count, tweet.retweet_count, tweet.created_at, cleaned_text, tweet.entities['media'][0]['media_url'], [idx]] for idx,tweet in enumerate(alltweets)]
    outputfile = '/home/linda/manifestos/tweets/downloads/NL/Party leaders/test/{}_{}_tweets.csv'.format(screen_name,lidmaatschap[screen_name])
    print("Writing stuff to {}".format(outputfile))
    with open(outputfile, 'w') as f:
        writer = csv.writer(f)
        writer.writerow(["id","favourite_count","retweet_count", "created_at","text", "images"])
